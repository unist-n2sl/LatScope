...
class ebpfConf:
	def __init__(self):
		self.file_list = ["conf/management_server_info.yaml", "conf/sampling_info.yaml", "conf/connect_info.yaml", "conf/database_info.yaml", "conf/redis_info.yaml", "conf/time_sync.yaml"]
	...
	def __read_conf__(self):
		for idx, file_name in enumerate(self.file_list):
			with open(file_name) as f:
				...
				elif idx == 5: self.time_sync = yaml.load(f, Loader = yaml.FullLoader)
