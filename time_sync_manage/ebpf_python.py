...
######################################################################
    def __xdp_event__(self, cpu, data, size):
        ...
        print( "recv_ts: %d send_ts: %d server_ts: %d " % (data.recv_ts, data.send_ts, data.server_ts)) 
...
#######################################################################
    def __attach_function__(self):
        ...
        for eth in eths:
            attach_ret = self.b.attach_xdp(eth, in_fn, 0)
            print(f'interface: {eth} - {attach_ret}')
#           self.b.attach_xdp(eth, in_fn, 1 << 1)
