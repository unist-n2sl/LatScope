class ebpfCode:
    ...
    def __set_common_func__(self):
        return r'''
            ...
            static u64 atoi(struct iphdr *iphdr, struct udphdr *udphdr, void *data_end) {
                u64 ts = 0;
                u8* p = 0;
            ...
            static void event_occur(struct xdp_md *ctx, struct iphdr *iphdr, struct udphdr *udphdr, void * data_end) {
                bpf_trace_printk("event_occur - management\n");
                ...
                u64 server_ts = atoi(iphdr, udphdr, data_end);
    ...
    def __set_main_func__(self):
        return r'''
            ...
            int syncTimeProtocol(struct xdp_md *ctx) {
                ...
                bpf_trace_printk("saddr: %u, sport: %u, dport: %u\n", iphdr->saddr, udphdr->source, udphdr->dest);
                ...
                if (iphdr->daddr == LOCAL_HOST && (check_send(bpf_ntohs(udphdr->dest)))) {
                    ...
                    udphdr->check = 0;
                    //ipv4_l4_csum(udphdr, 8, &cs, iphdr);
                ...
                else if (check_recv(iphdr->saddr, bpf_ntohs(udphdr->source))) {
                    event_occur(ctx, iphdr, udphdr, data_end);
                    return XDP_DROP;
                }
                return XDP_PASS;
            } 
